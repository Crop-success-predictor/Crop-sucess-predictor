...
def api_for_weather(place):
    #print(place)
    result=requests.get('https://api.openweathermap.org/data/2.5/weather?q=%27'+place+'%27&appid=cf4a2f34b5698a0066b9de7f55e2f4c6')
    data=result.json()
    return data
...
def api_for_weather_2(lat,longi):
    ...
    #print(lat)
   # print(longi)
    result=requests.get('http://api.openweathermap.org/data/2.5/weather?lat='+str(lat)+'&lon='+str(longi)+'&appid=cf4a2f34b5698a0066b9de7f55e2f4c6')
...
def get_soil_info(place,coord):
    ...
    #    result=requests.get('https://api.weatherbit.io/v2.0/history/agweather?lat='+str(coord[0])+'&lon='+str(coord[1])+'&start_date='+start_date+'&end_date='+end_date+'&key=283a77fe5cbe46718430e4d5418be6c1')
        result=requests.get('https://api.weatherbit.io/v2.0/forecast/daily?&lat='+str(coord[0])+'&lon='+str(coord[1])+'&key=18d41d5ee2ab47afbc189653c43d4fb8')
        data=result.json()
        print(data)
        return data
    ...
    def cal_mean(data):
        mean_vals=[]
        summ_vals_dict={}
        keywords=['bulk_soil_density','temp','precip','specific_humidity','pres','soilt_0_10cm','soilm_0_10cm','wind_spd']
        for i in range(0,9):
            data['data'][i]['bulk_soil_density']=1390
            data['data'][i]['specific_humidity']=0.0032899952493608
            data['data'][i]['soilt_0_10cm']=19.9
            data['data'][i]['soilm_0_10cm']=14.804

        for k in keywords:
            summ=0
            for i in range(0,9):
                #print(k,data['data'][i][k])

                summ+=data['data'][i][k]
            summ_vals_dict[k]=summ
        for key in summ_vals_dict:
            summ_vals_dict[key]=(summ_vals_dict[key])
    
        #print(summ_vals_dict)
...
def work(request):
    if request.method=='POST':
        ...
        if (prod_area.objects.filter(crop=crop,district=area)).count()>=1:
            ...
            #print(area)
            #print(prod_area.objects.filter(crop=crop,district=area))
            ...
            #print(coord)                 #extracting the coordinates
            soil_info=get_soil_info(area,coord) 
            #print(soil_info)         #dictionary cointaining soil info
            temp_det=print_temp_details(data) 
            #print(temp_det)       #converting all the details into proper units for printing
